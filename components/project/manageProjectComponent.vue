  ...
  <div>
    ...
    <el-table :data="tableData" v-loading="isLoading" style="width: 100%">
      <el-table-column
        ...
        label="STT"
      />
      <el-table-column align="center" prop="title_large" label="Tiêu đề lớn" />
      <el-table-column align="center" label="Ảnh đại diện" width="200">
        <template #default="scope">
          <img :src="`${config.public.apiBase}${scope.row.image}`" />
        </template>
      </el-table-column>
      <el-table-column align="center" prop="title_short" label="Tiêu đề nhỏ" />
      <el-table-column align="center" prop="des_short" label="Mô tả ngắn" />
      ...
      >
        <template #default="scope">
          <div class="d-flex justify-content-center">
            <nuxt-link :to="'/project/edit/' + scope.row._id">
              <el-tooltip placement="top" content="Sửa dự án">
            ...
            <el-tooltip placement="top" content="Xóa dự án">
              <span
...
watch(
  ...
  async (value: any) => {
    if (value) {
      ...
      try {
        const res: any = await useBaseFetch(
          `/project/${focusID.value}/delete`,
          {
            method: "DELETE",
          }
        );
...
const handleGetData = async () => {
  ...
  const res: any = (await useBaseFetch("/project/list", {
    method: "GET",
    query,
  })) as any;
